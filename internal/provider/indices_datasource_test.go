...
func TestAccDataSourceBalancedStorageIndices(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Create an index with balanced storage to ensure we have data to read
			{
				Config: testAccDataSourceBalancedIndexConfig(balanced_storage_index_name, "marqo.balanced.storage"),
				Check: resource.ComposeTestCheckFunc(
					func(s *terraform.State) error {
			...
			// Read the index using the data source
			{
				Config: testAccDataSourceIndicesConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBalancedIndexInDataSource("data.marqo_read_indices.test", balanced_storage_index_name, "marqo.balanced.storage"),
				),
			},
		},
	})
...
func TestAccDataSourceBalancedThroughputIndices(t *testing.T) {
	t.Parallel()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Create an index with balanced throughput to ensure we have data to read
			{
				Config: testAccDataSourceBalancedIndexConfig(balanced_throughput_index_name, "marqo.balanced.throughput"),
				Check: resource.ComposeTestCheckFunc(
					func(s *terraform.State) error {
			...
			// Read the index using the data source
			{
				Config: testAccDataSourceIndicesConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBalancedIndexInDataSource("data.marqo_read_indices.test", balanced_throughput_index_name, "marqo.balanced.throughput"),
				),
			},
		},
	})
...
func testAccDataSourceBalancedIndexConfig(name string, storageClass string) string {
	return fmt.Sprintf(`
		resource "marqo_index" "test" {
		index_name = "%s"
		timeouts = {
...
func testAccCheckBalancedIndexInDataSource(dataSourceName string, indexName string, expectedStorageClass string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		ds, ok := s.RootModule().Resources[dataSourceName]
		if !ok {
			return fmt.Errorf("Data source not found: %s", dataSourceName)
		...
		for i := 0; i < itemCount; i++ {
			if ds.Primary.Attributes[fmt.Sprintf("items.%d.index_name", i)] == indexName {
				...
				attributesToCheck := map[string]string{
					"inference_type": "marqo.CPU.small",
					"storage_class":  expectedStorageClass,
					// Add more attributes to check
				}
